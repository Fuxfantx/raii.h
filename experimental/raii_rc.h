...
typedef struct {
	...
	const RAII_THREAD_SAFE_H_FINAL	RAII_RC_H_FINAL;
...
static inline RAII_RC_H_RC_PTR_OPAQUE_DATA_STRUCTURE RAII_RC_H_RC_PTR_NEW_WITH_FINAL(const uint64_t size, const RAII_THREAD_SAFE_H_FINAL final) {
...
#ifndef Ref
#define Ref(handle, ptype, name)  USE_TRAII() TSCOPE(RAII_RC_H_RC_PTR_OPAQUE_DATA_STRUCTURE*, RAII_RC_H_CURRENT_RCPTR_HANDLE, (RcPtrRef(& handle ),& handle), RcPtrDeref)         \
