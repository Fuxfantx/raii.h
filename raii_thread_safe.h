...
static inline void* RAII_THREAD_SAFE_H_REGISTER( RAII_THREAD_SAFE_H_CHAIN** RAII_THREAD_SAFE_H_CHAIN_CURRENT, void** final_which, const RAII_THREAD_SAFE_H_FINAL final ) {
    RAII_THREAD_SAFE_H_CHAIN* const latest = malloc(sizeof(RAII_THREAD_SAFE_H_CHAIN));
...
static inline unsigned char RAII_THREAD_SAFE_H_TRY_FINAL(RAII_THREAD_SAFE_H_CHAIN** RAII_THREAD_SAFE_H_CHAIN_CURRENT) {
    if( RAII_THREAD_SAFE_H_CHAIN_CURRENT && ++(*RAII_THREAD_SAFE_H_CHAIN_CURRENT)->tag ) {
        ...
        free(*RAII_THREAD_SAFE_H_CHAIN_CURRENT), *RAII_THREAD_SAFE_H_CHAIN_CURRENT = temporary_parent;
        return 0;
    }
    return 1;
}

/* 1. Use "RAII_MALLOC" instead of "malloc" to express that the memory you allocated will be freed in a RAII way. */
#ifndef RAII_MALLOC
#define RAII_MALLOC  malloc
#endif

/* 2. Before creating the 1st RAII scope, use the "USE_RAII();" clause to properly initialize the RAII handle. */
#ifndef USE_TRAII
#define USE_TRAII()  RAII_THREAD_SAFE_H_CHAIN* RAII_THREAD_SAFE_H_CHAIN_CURRENT = 0;
#endif

/* 3A.Typed ptr with with a scoped destructor */
#ifndef TRAII
...
/* 3B.Typed ptr with scoped constructor & destructor */
#ifndef TSCOPE
...
/* 4. Use Treturn instead of "return" to ensure finalizers called correctly. */
